...
The maintained filtering contract has four options:
...
```ini
[https://fed.dyne.org/feeds/c/cybersec.xml?sort=New]
name = cybersec
lemmy = true
```
...
- Lemmy rewriting runs before the post-normalization `regexp`, `sed`, and `excerpt` steps.
- Entries are then processed in a fixed order: `regexp`, then `sed`, then `excerpt`.
